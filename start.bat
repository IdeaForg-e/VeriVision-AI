...
echo [1/2] Seeding database and starting backend on http://localhost:8000
start "VERIVISION-AI Backend" cmd /k "cd /d "%BACKEND_DIR%" && "%BACKEND_PY%" seed_data.py && "%BACKEND_PY%" -m uvicorn app.main:app --reload --host 127.0.0.1 --port 8000"
